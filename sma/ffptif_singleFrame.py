...
def ffp_tifSF(filename,xmlname):    
    ...
    if par.d3peaks == 1:
        ...
        currset_st=0
        frames = np.zeros((ycrop,xcrop,0)) #note: x value is column, y value is row.
        print ("number sets: %i" %no_sets)
        for i in range(0,0): #goes from 0 to no_sets -1
            ...
            if par.keeptype == 0: 
                ...
                keep = ffpslice.ffp_keep(medimg,0,par,active[:,0:no_a])
                for a in range(0,no_a):
    ...
    if par.autocont==1:
        ...
        #print 'apdax.py'
        from aptif_singleFrame import ap_tifSF
        ap_tifSF(filename,xmlname)
